...
(async () => {
  ...
  setInterval(async () => {
    ...
    const onceTasks = await database.task.findMany({where: {finished: false, occurenceRate: OccurenceRate.Once, OR: [{nextScheduledExecutionDate: {lte: new Date()},}, {nextScheduledExecutionDate: undefined}]}, include: {frequency: true}});
    ...
    const recurringTasks = await database.task.findMany({where: {finished: false, occurenceRate: OccurenceRate.Recurring, OR: [{nextScheduledExecutionDate: {lte: new Date()},}, {nextScheduledExecutionDate: undefined}]}, include: {frequency: true}});
